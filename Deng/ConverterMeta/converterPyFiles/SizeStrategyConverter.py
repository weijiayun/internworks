...
class SizeStrategyMeta(OrderedObject):
    ...
    def get_const_member_list(self):
        member_list = ['Name','Type','MapSignal','IsMapSignal',
            'SizeSignal','SumMaxCap','TargetInstrumentFile',
            'TargetPositionTime','ActionTimeInterval','SellToBuyWaitTime',
            'IndexFeedCode','BalanceDay','MaxInstrument',
...
class SizeStrategyConverter(object):
    def __init__(self, work_dir=None):
        self.id = float('nan')
        self.__SizeStrategy_meta_list = []
        self.work_dir = work_dir
        self.__bp_manager = None
        
    def set_meat_list(self, HeFeiV3_meta_list):
        self.__SizeStrategy_meta_list = HeFeiV3_meta_list

    def get_meat_list(self):
        return self.__SizeStrategy_meta_list
    ...
    def start_converter(self):
        ...
        for meta in self.__SizeStrategy_meta_list:
            self.strategy_to_json(meta)

    def strategy_to_json(self, SizeStrategyMeta):
        data_json = {}
        try:
            signal_finder = SignalFinder()
            for member in SizeStrategyMeta.get_const_member_list():
                member_value = getattr(SizeStrategyMeta, member)
                if member in SizeStrategyMeta.get_portfolio_list():
                    data_json[member] = reference_portfolio(member_value)
                    if data_json[member] == -1:
                        raise CSVConverterError(SizeStrategyMeta.Name, SizeStrategyMeta.Type, "Can't find portfolio: {0}".format(member_value))
                elif member in SizeStrategyMeta.get_signal_list():
                    data_json[member] = (signal_finder.get_signal_id_by_name(member_value), member_value)[isinstance(member_value, int)]
                    if data_json[member] == -1:
                        raise CSVConverterError(SizeStrategyMeta.Name, SizeStrategyMeta.Type, "Can't find signal: {0}".format(member_value))

                elif member in SizeStrategyMeta.get_bool_list():
                    data_json[member] = is_bool(member_value)
                elif member in SizeStrategyMeta.get_MatchStrategyId():
                    data_json[member] = (get_strategy_id_by_name(self.work_dir, member_value), member_value)[isinstance(member_value, int)]
                    if data_json[member] == -1:
                        raise CSVConverterError(SizeStrategyMeta.Name, SizeStrategyMeta.Type, "Can't find matchstrategy:{0}".format(member_value))
                else:
                    data_json[member] = member_value
            data_json['Instruments'] = generate_instruments(SizeStrategyMeta, SizeStrategyMeta.member_list)
            data_json['Ranges'] = generate_ranges(SizeStrategyMeta, SizeStrategyMeta.member_list)
        except Exception as e:
            raise CSVConverterError(SizeStrategyMeta.Name, SizeStrategyMeta.Type, e.message)
