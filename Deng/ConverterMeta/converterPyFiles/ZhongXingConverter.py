            ...
            for member in ZhongXingMeta.get_const_member_list():
                ...
                if member in ZhongXingMeta.get_portfolio_list():
                    ...
                    if data_json[member] == -1:
                        raise CSVConverterError(ZhongXingMeta.Name, ZhongXingMeta.Type, "Can't find portfolio: {0}".format(member_value))
                    ...
                    if data_json[member] == -1:
                        raise CSVConverterError(ZhongXingMeta.Name, ZhongXingMeta.Type, "Can't find signal: {0}".format(member_value))
                    ...
                    if data_json[member] == -1:
                        raise CSVConverterError(ZhongXingMeta.Name, ZhongXingMeta.Type, "Can't find matchstrategy:{0}".format(member_value))
                else:
                    data_json[member] = member_value
            data_json['Instruments'] = generate_instruments(ZhongXingMeta, ZhongXingMeta.member_list)
            data_json['Ranges'] = generate_ranges(ZhongXingMeta, ZhongXingMeta.member_list)
        except Exception as e:
            raise CSVConverterError(ZhongXingMeta.Name, ZhongXingMeta.Type, e.message)
