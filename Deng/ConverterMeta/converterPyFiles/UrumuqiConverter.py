...
class UrumuqiMeta(OrderedObject):
    def __init__(self):
        super(UrumuqiMeta,self).__init__()
        self.Name = ""
        self.Type = "Urumuqi"
        self.MoneynessLowerBound = ""
        self.MoneynessUpperBound = ""
    ...
    def get_const_member_list(self):
        member_list = ['Name','Type','MoneynessLowerBound','MoneynessUpperBound',
            'LossThresholdForClosePosition','LossThresholdForOpenPosition','DaysToMaturityForClosePosition',
            'DaysToMaturityForOpenPosition','StopLossRatio','TargetPosition',
            'UnderlyingInstName','ActionTime','DependentSignalId',
            'MinMaxPositionChangePercent','XN','XD',
            'IsNightTrade','IsDayTrade','OrderType',
    ...
    def required_member_list(self):
        required_list=['Name', 'Type', 'MoneynessLowerBound', 'MoneynessUpperBound', 'LossThresholdForClosePosition', 'LossThresholdForOpenPosition', 'DaysToMaturityForClosePosition', 'DaysToMaturityForOpenPosition', 'StopLossRatio', 'TargetPosition', 'UnderlyingInstName', 'ActionTime', 'DependentSignalId', 'MinMaxPositionChangePercent', 'XN', 'XD', 'EmitterType', 'Instruments_InstrumentName1', 'Instruments_IsConnect1', 'Instruments_Market1', 'Portfolio']
...
class UrumuqiConverter(object):
    def __init__(self, work_dir=None):
        self.id = float('nan')
        self.__Urumuqi_meta_list = []
        self.work_dir = work_dir
        self.__bp_manager = None
        
    def set_meat_list(self, HeFeiV3_meta_list):
        self.__Urumuqi_meta_list = HeFeiV3_meta_list

    def get_meat_list(self):
        return self.__Urumuqi_meta_list
    ...
    def start_converter(self):
        ...
        for meta in self.__Urumuqi_meta_list:
            self.strategy_to_json(meta)

    def strategy_to_json(self, UrumuqiMeta):
        data_json = {}
        try:
            signal_finder = SignalFinder()
            for member in UrumuqiMeta.get_const_member_list():
                member_value = getattr(UrumuqiMeta, member)
                if member in UrumuqiMeta.get_portfolio_list():
                    data_json[member] = reference_portfolio(member_value)
                    if data_json[member] == -1:
                        raise CSVConverterError(UrumuqiMeta.Name, UrumuqiMeta.Type, "Can't find portfolio: {0}".format(member_value))
                elif member in UrumuqiMeta.get_signal_list():
                    data_json[member] = (signal_finder.get_signal_id_by_name(member_value), member_value)[isinstance(member_value, int)]
                    if data_json[member] == -1:
                        raise CSVConverterError(UrumuqiMeta.Name, UrumuqiMeta.Type, "Can't find signal: {0}".format(member_value))

                elif member in UrumuqiMeta.get_bool_list():
                    data_json[member] = is_bool(member_value)
                elif member in UrumuqiMeta.get_MatchStrategyId():
                    data_json[member] = (get_strategy_id_by_name(self.work_dir, member_value), member_value)[isinstance(member_value, int)]
                    if data_json[member] == -1:
                        raise CSVConverterError(UrumuqiMeta.Name, UrumuqiMeta.Type, "Can't find matchstrategy:{0}".format(member_value))
                else:
                    data_json[member] = member_value
            data_json['Instruments'] = generate_instruments(UrumuqiMeta, UrumuqiMeta.member_list)
            data_json['Ranges'] = generate_ranges(UrumuqiMeta, UrumuqiMeta.member_list)
        except Exception as e:
            raise CSVConverterError(UrumuqiMeta.Name, UrumuqiMeta.Type, e.message)
